      ...
      <div className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-foreground ">Reading Test Results</h1>
      ...
      <Card className="mb-8">
        <CardContent className="pt-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h2 className="text-xl font-medium text-foreground ">Test Completed Successfully! 🎉</h2>
              <p className="text-muted-foreground ">Your IELTS Academic Reading test has been evaluated.</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="mb-8">
        <CardHeader className="text-center bg-muted/30 py-6">
          <div className="flex items-center justify-center gap-3">
            ...
            <CardTitle className="text-xl font-medium text-foreground ">Your IELTS Reading Band Score</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="text-center py-8">
          <div className="text-7xl font-bold mb-6 text-foreground ">
            {estimatedBandScore}
          </div>
          ...
          <div className="inline-flex items-center px-6 py-3 rounded-full bg-primary text-primary-foreground font-medium text-lg ">
          ...
          <div className="flex justify-center">
            <div className="flex bg-muted/50 rounded-xl p-2 gap-2">
              {Object.keys(testParts).map(partNum => {
                ...
                return <Button key={partNumber} variant={currentPart === partNumber ? "default" : "ghost"} size="sm" onClick={() => setCurrentPart(partNumber)} className="min-w-[120px] rounded-lg font-medium ">
                  Part {partNumber}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {Object.entries(questionTypes).map(([type, count]) => <div key={type} className="flex justify-between items-center p-3 rounded-lg bg-muted/30">
              <span className="text-sm capitalize font-medium text-foreground ">{type.replace('_', ' ')}</span>
              <span className="text-xl font-semibold text-foreground ">
                {count}
              </span>
            </div>)}
        ...
        <Card className="flex flex-col w-[50%] h-full">
          ...
          <CardContent className="flex-1 overflow-y-auto min-h-0 p-6">
            <div className="prose prose-sm max-w-none">
              <div className="whitespace-pre-wrap leading-relaxed select-text text-foreground " id={`passage-content-${currentPart}`} style={{
                fontSize: '16px',
                lineHeight: '1.8'
              }}>
