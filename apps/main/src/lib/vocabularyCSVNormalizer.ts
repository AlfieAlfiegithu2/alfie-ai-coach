...
function normalizeQuotes(s: string) {
  return s
    .replace(/[\u201C\u201D]/g, '"') // curly double to straight
    .replace(/[\u2018\u2019]/g, "'"); // curly single to straight
}
...
function ensureBlank(content: string, correct: string, warnings: string[]): { content: string; error?: string } {
  ...
  if (blanks.length === 0) {
    // try replace the correct answer occurrence
    ...
    // otherwise, insert before last punctuation
  ...
  // normalize to exactly 7 underscores
...
export function normalizeVocabularyCSV(
  file_text: string,
  skill_type: "Vocabulary Builder",
  skill_test_id: string
): NormalizedOutput {
      ...
      errors: [
        {
          row: 0,
          message:
            `Missing headers. Expected: ${REQUIRED_HEADERS.join(",")}`,
          raw: {},
        },
      ],
      summary: {
  ...
  rows.forEach((line, idx) => {
    const rowNumber = idx + 1; // 1-based for data rows, but header is line 0 in source
...
const rawObj = {
  ...
  Explanation,
    ...
    // classify format (case-insensitive tolerant)
    ...
    if (QuestionFormat === "DefinitionMatch") {
      // content should be single word ideally
      ...
      // replace low-quality distractors if obviously empty/unrelated
      ...
      // distractors: single words, same POS where possible
    ...
    // Deduplicate and ensure 3 incorrects
