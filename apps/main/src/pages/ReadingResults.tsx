...
import { BookOpen, ArrowLeft, CheckCircle, XCircle, Clock, Trash2, Eye, ChevronDown, ChevronRight, Search, Filter, TrendingDown, Bookmark, BookmarkCheck } from "lucide-react";
...
const ReadingResults = () => {
  ...
  const [processedResultsMap, setProcessedResultsMap] = useState<{
    [key: string]: ProcessedQuestion[];
  }>({});
  ...
  const loadBookmarks = async () => {
    ...
    try {
      const {
        data: bookmarks,
        error
      } = await supabase.from('question_bookmarks').select('question_id, test_result_id').eq('user_id', user.id);
      if (error) {
        console.error('Error fetching bookmarks:', error);
        return;
      }
      const bookmarkKeys = new Set(bookmarks?.map(b => `${b.test_result_id}-${b.question_id}`) || []);
  ...
  const toggleBookmark = async (result: ReadingResult, question: ProcessedQuestion) => {
    ...
    try {
      if (isCurrentlyBookmarked) {
        ...
        const {
          error
        } = await supabase.from('question_bookmarks').delete().eq('user_id', user.id).eq('question_id', question.questionNumber.toString()).eq('test_result_id', result.id);
  ...
  const filteredResults = results.filter(result => {
    ...
    if (searchTerm) {
      ...
      const questionMatch = visibleQuestions.some(q => q.question.toLowerCase().includes(searchLower) || q.userAnswer.toLowerCase().includes(searchLower) || q.correctAnswer.toLowerCase().includes(searchLower));
    ...
    if (filterType !== 'all') {
      if (filterType === 'bookmarked') {
        ...
        const hasBookmarkedQuestion = visibleQuestions.some(q => isQuestionBookmarked(result.id, q));
        if (!hasBookmarkedQuestion) return false;
      } else {
        // Filter by question type
        const hasFilterType = visibleQuestions.some(q => q.type?.toLowerCase().includes(filterType.toLowerCase()));
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {results.length > 0 && <div className="mb-8 space-y-4">
            <div className="flex gap-4 items-center">
              <div className="relative flex-1 max-w-md">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground w-4 h-4" />
                <Input placeholder="Search questions, answers, or passages..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
              </div>
            ...
            <div className="flex gap-6 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground">Incorrect Notes: <span className="text-foreground font-medium">{filteredResults.reduce((acc, result) => {
                  const questions = processedResultsMap[result.id] || [];
                  const visibleQuestions = questions.filter(q => !isQuestionRemoved(result.id, q.questionNumber));
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground">Bookmarked: <span className="text-foreground font-medium">{results.reduce((count, result) => {
                  const questions = processedResultsMap[result.id] || [];
        ...
        <div className="space-y-6">
          ...
          return <Collapsible key={result.id} open={isExpanded} onOpenChange={() => toggleResultExpansion(result.id)}>
                    ...
                    <CollapsibleTrigger className="w-full">
                      <CardHeader className="hover:bg-muted/30 transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="text-left">
                              <CardTitle className="text-lg font-semibold text-foreground flex items-center gap-2">
                                ...
                                {visibleQuestions.some(q => isQuestionBookmarked(result.id, q)) && <Badge variant="outline" className="text-xs text-yellow-600 border-yellow-200">
                          ...
                          <div className="flex items-center gap-2">
                            {visibleQuestions.length > 0 && <Button variant="outline" size="sm" onClick={e => {
                        e.stopPropagation();
                    ...
                    <CollapsibleContent>
                      <CardContent className="pt-0 px-6 pb-6">
                        <div className="space-y-4">
                          {visibleQuestions.map((question, index) => <div key={index} className={`p-4 border rounded-lg hover:shadow-sm transition-all ${isQuestionBookmarked(result.id, question) ? 'bg-yellow-50 border-yellow-200' : 'bg-red-50 border-red-200'}`}>
                              <div className="flex items-start gap-3">
                                <div className="flex items-center gap-2 flex-shrink-0">
                                  ...
                                  {isQuestionBookmarked(result.id, question) && <BookmarkCheck className="w-4 h-4 text-yellow-600" />}
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  {question.options && question.options.length > 0 && <div className="mb-3 p-2 bg-background/60 rounded border border-border/50">
                                      <div className="grid grid-cols-1 gap-1">
                                        {question.options.map((option, optIndex) => <div key={optIndex} className="text-xs text-foreground/80">
                                            {option}
                                          </div>)}
                                      </div>
                                    </div>}
                                  ...
                                  {question.explanation && <div className="mt-2 p-2 bg-blue-50/80 rounded border border-blue-200/50">
                          ...
                          }} className={`h-8 w-8 p-0 ${isQuestionBookmarked(result.id, question) ? 'text-yellow-600 hover:text-yellow-700 hover:bg-yellow-100' : 'text-gray-400 hover:text-yellow-600 hover:bg-yellow-50'}`}>
                                    {isQuestionBookmarked(result.id, question) ? <BookmarkCheck className="w-4 h-4" /> : <Bookmark className="w-4 h-4" />}
                                  </Button>
                                  <Button variant="ghost" size="sm" onClick={() => handleQuestionClick(question, result.passage_text || '')} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700 hover:bg-blue-50">
                                    <Eye className="w-4 h-4" />
                                  </Button>
                                  <Button variant="ghost" size="sm" onClick={() => removeQuestion(result.id, question.questionNumber)} className="h-8 w-8 p-0 text-red-600 hover:text-red-700 hover:bg-red-50">
        ...
        <QuestionReviewModal isOpen={!!selectedQuestion} onClose={() => setSelectedQuestion(null)} question={selectedQuestion} passage={selectedPassage} />
      </div>
    </div>;
};
