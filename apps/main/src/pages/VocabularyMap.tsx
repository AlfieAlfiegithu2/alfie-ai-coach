...
const VocabularyMap = () => {
  ...
  useEffect(() => {
    document.title = "Vocabulary Map | Vocabulary Builder";
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Load tests in creation order (oldest first = level 1)
      ...
      if (!uid) {
        // Not logged in: default to level 1
        setUnlocked(1);
        setLoading(false);
        return;
      }

      const { data: prog, error } = await (supabase as any)
      ...
      if (!error && prog) {
        setUnlocked(Math.max(1, prog.max_unlocked_level || 1));
      } else if (!error && !prog && uid) {
        // initialize row
        await (supabase as any)
          .from("user_skill_progress")
          .insert({ user_id: uid, skill_slug: "vocabulary-builder", max_unlocked_level: 1 });
        setUnlocked(1);
      }
      setLoading(false);
    };
  ...
  const nodes = useMemo(() => {
    ...
    for (let r = 0; r < GRID_ROWS; r++) {
      ...
      // Zig-zag path like Mario: reverse every other row
  ...
  return (
    <StudentLayout title="Vocabulary Map" showBackButton backPath="/skills/vocabulary-builder">
      <main className="max-w-4xl mx-auto space-y-6">
        <section aria-label="Map progress" className="space-y-2">
          <div className="text-sm text-muted-foreground">Progress to the castle</div>
          ...
          <CardContent className="p-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={unlockNext} aria-label="Manually unlock next level">Unlock Next</Button>
              </div>
            </div>
      ...
      <script type="application/ld+json" suppressHydrationWarning>
        {JSON.stringify({
          ...
          "@type": "CollectionPage",
          name: "Vocabulary Builder Map",
          description: "Mario-style 20-level vocabulary progression ending at a castle.",
          isPartOf: { "@type": "WebSite", name: "Vocabulary Builder" },
        })}
      </script>
    </StudentLayout>
  );
