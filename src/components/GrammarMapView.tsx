...
const GrammarMapView = () => {
  ...
  // Grammar-themed progression with images
  ...
  const getAnimalForLevel = (levelIndex: number) => {
    if (levelIndex < grammarProgression.length) {
      return grammarProgression[levelIndex];
    }
    return grammarProgression[grammarProgression.length - 1];
  };
  ...
  const loadMapData = async () => {
    ...
    try {
      ...
      toast.error('Failed to load grammar map');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleNodeClick = (node: MapNode) => {
    navigate(`/skills/grammar-fix-it/test/${node.test.id}`);
  };
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Card className="p-8 text-center">
          <CardContent>
            <p className="text-lg mb-4">Please sign in to access your grammar journey.</p>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Do you want to unlock this level? This will allow you to start the {getAnimalForLevel(index).name} grammar test.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
