...
const Header = () => {
  ...
  return <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 max-w-screen-2xl items-center justify-between px-4">
        ...
        <div className="flex items-center space-x-2">
          {!user && <Button variant="outline" onClick={() => navigate('/auth')} className="text-sm font-medium">
              Log In
            </Button>}
          
          {user && <Button variant="outline" onClick={() => navigate('/personal-page')} className="text-sm font-medium">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          ...
          <Button variant="ghost" onClick={() => navigate('/community')} className="text-sm font-medium transition-colors hover:text-primary">
            Community
          </Button>
        </nav>
        ...
        <div className="flex items-center space-x-4">
          {user ? <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                ...
                {admin && <DropdownMenuItem onClick={() => navigate('/admin')}>
              ...
              <Button onClick={() => navigate('/auth')} variant="outline" size="sm" className="text-sm font-medium border-white/20 text-foreground hover:bg-primary/10">
                Log In
              </Button>
              <Button onClick={() => navigate('/auth')} className="text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90">
                Sign Up
              </Button>
            </div>}

          {/* Admin Button - only show if not already admin */}
          {!admin && <Button variant="outline" size="sm" onClick={() => navigate('/admin/login')} className="hidden md:flex text-xs">
              Admin
            </Button>}
