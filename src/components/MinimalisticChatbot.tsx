...
const MinimalisticChatbot: React.FC<MinimalisticChatbotProps> = ({
  ...
  return <div className="fixed bottom-6 right-6 z-50">
      <Card className={`backdrop-blur-lg bg-background/30 border-border/20 shadow-xl transition-all duration-300 ${isMinimized ? 'w-80 h-16' : getChatDimensions()}`}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-border/30">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={selectedLanguage} onValueChange={onLanguageChange}>
              ...
              <SelectContent>
                {languages.map(lang => <SelectItem key={lang.code} value={lang.code} className="text-xs">
                    {lang.name}
                  </SelectItem>)}
              </SelectContent>
            </Select>
        ...
        {!isMinimized && <>
            ...
            <div className="px-4 py-2 border-b border-border/30">
              <Select value={selectedTopic} onValueChange={setSelectedTopic}>
                ...
                <SelectContent>
                  {topics.map(topic => <SelectItem key={topic.value} value={topic.value} className="text-xs">
                      {topic.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
          ...
          height: chatSize === 'small' ? '16rem' : chatSize === 'large' ? '24rem' : '20rem'
              ...
              {messages.map(message => <div key={message.id} className={`flex gap-2 ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                  {!message.isUser && <div className="flex-shrink-0">
                      <Bot className="h-6 w-6 text-brand-blue" />
                    </div>}
                  <div className={`max-w-[75%] p-3 rounded-2xl text-sm ${message.isUser ? 'bg-brand-blue text-white' : 'glass-effect border border-border/30'}`}>
                    <p className="leading-relaxed">{message.text}</p>
                    {!message.isUser && selectedLanguage !== 'en' && <Button variant="ghost" size="sm" onClick={() => translateText(message.text)} className="mt-2 h-6 text-xs text-brand-blue hover:text-brand-blue/80">
            ...
            <div className="p-4 border-t border-border/30">
              <div className="flex gap-2">
                <Input value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder={chatSize === 'small' ? "Type here..." : "Ask me anything about English..."} className="flex-1 border-border/30 bg-surface-1/80 backdrop-blur-sm text-sm min-h-[40px]" disabled={isLoading} />
                <Button onClick={handleSendMessage} disabled={isLoading || !inputMessage.trim()} className="h-10 w-10 p-0 bg-brand-blue hover:bg-brand-blue/90 flex-shrink-0">
