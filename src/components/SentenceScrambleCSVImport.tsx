...
import { normalizeSentenceScrambleCSV, type NormalizedOutput } from "@/lib/sentenceScrambleCSVNormalizer";
...
export default function SentenceScrambleCSVImport({ skillTestId, onImported }: Props) {
  ...
  const handleFile = async (file: File) => {
    try {
      ...
      const result = normalizeSentenceScrambleCSV(text, "Sentence Structure Scramble", skillTestId);
  ...
  const downloadTemplate = () => {
    ...
    const sample = [
      'SentenceScramble,"The company will launch a new product next month.","Unscramble the sentence.","The company","will launch","a new product","next month.","This order follows Subject + Modal/Verb + Object + Time."',
    ].join("\n");
    const blob = new Blob([header + sample], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "sentence-scramble-template.csv";
    a.click();
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Upload Sentence Scramble CSV</CardTitle>
      </CardHeader>
            ...
            <div className="max-h-60 overflow-auto rounded border p-2 text-xs">
              {preview.slice(0, 10).map((r, i) => (
                <div key={i} className="py-1 border-b last:border-b-0">
                  ...
                  <div className="text-muted-foreground"><span className="font-medium">Original:</span> {r.original_sentence}</div>
