...
const AdminGrammarTests = () => {
  ...
  useEffect(() => {
    document.title = "Grammar Fix-it Tests | Admin";
    load();
  }, []);
  ...
  const addTest = async () => {
    ...
    const { data, error } = await (supabase as any)
      .from("skill_tests")
      .insert({ skill_slug: "grammar-fix-it", title: title.trim() })
      .select("id")
      .maybeSingle();
    setLoading(false);
    if (error) {
    ...
    setTitle("");
    await load();
    if (data?.id) navigate(`/admin/skills/grammar/tests/${data.id}`);
  };

  return (
    <AdminLayout title="Grammar Fix-it Tests" showBackButton>
      <section className="space-y-4">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">+ Add New Grammar Test</CardTitle>
          </CardHeader>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {tests.map((t) => (
            <Card key={t.id} className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <Button size="sm" variant="secondary" onClick={() => navigate(`/admin/skills/grammar/tests/${t.id}`)}>
                  Manage Test
                </Button>
              </CardContent>
            </Card>
