...
import ParaphrasingCSVImport from "@/components/ParaphrasingCSVImport";
import ParaphraseQuizPreview from "@/components/ParaphraseQuizPreview";
...
const AdminParaphrasingTestDetail = () => {
  const { id } = useParams();
  ...
  return (
    <AdminLayout title={test ? `Manage: ${test.title}` : "Manage Paraphrasing Test"} showBackButton>
      <section className="space-y-6">
        {id && (
          <ParaphrasingCSVImport skillTestId={id} onImported={load} />
        )}
        ...
        <div ref={previewRef}>
          <Card>
            ...
            <CardContent>
              <ParaphraseQuizPreview questions={questions as any} selectedQuestionId={selectedQuestionId || undefined} />
            </CardContent>
          </Card>
        </div>
                ...
                <div key={q.id} className="p-3 border rounded-md space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    <div onClick={() => handlePreview(q.id)} className="cursor-pointer">
                      ...
                      {q.original_sentence && (
                        <div className="text-xs text-muted-foreground"><span className="font-medium">Original:</span> {q.original_sentence}</div>
                      )}
...
export default AdminParaphrasingTestDetail;
