...
import { Plus, Edit, Trash2, Upload, Eye, FileText, ChevronDown, ChevronRight, Search, BookOpen, Mic } from "lucide-react";
...
const AdminSpeaking = () => {
  ...
  const loadCambridgeStructure = async () => {
    try {
      // Load speaking prompts
      const promptsResult = await listContent('speaking_prompts');
  ...
  const handleUpload = async () => {
    try {
      ...
      await createContent('speaking_prompts', promptData);

      toast({
        title: "Success",
        description: `Speaking prompt uploaded to Cambridge ${uploadBookNumber}, Test ${uploadSectionNumber}`,
      });
  ...
  const deleteSection = async (bookNumber: number, sectionNumber: number) => {
    ...
    try {
      ...
      for (const prompt of section.prompts) {
        await deleteContent('speaking_prompts', prompt.id);
      }
  ...
  const saveEditingSection = async () => {
    ...
    try {
      ...
      for (const prompt of editingSection.prompts) {
        await updateContent('speaking_prompts', {
          id: prompt.id,
          title: prompt.title,
  ...
  return (
    <AdminLayout title="Cambridge IELTS Speaking">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-warm-gray">
              Manage speaking prompts organized by Cambridge books 1-20
            </p>
          </div>
          <Button
                                    ...
                                    <Dialog>
                                      ...
                                      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                                        ...
                                        <div className="space-y-6">
                                          <Card className="p-4">
                                            <h5 className="font-medium mb-3">Speaking Prompts ({section.prompts.length})</h5>
                                            <div className="space-y-4">
                                              {section.prompts.map((prompt: any) => (
                                                <div key={prompt.id} className="border-b border-light-border pb-4 last:border-b-0">
                                                  <div className="flex items-start gap-3">
                                                    <Badge variant="outline">Part {prompt.part_number}</Badge>
            ...
            <div className="space-y-6">
              ...
              <Card className="rounded-2xl border-light-border" style={{ background: 'white' }}>
                ...
                <CardContent className="space-y-4">
                  <Input
                    placeholder="Prompt Title (e.g., 'Part 2: Describe a memorable journey')"
                    value={promptTitle}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Select value={partNumber.toString()} onValueChange={(value) => setPartNumber(parseInt(value))}>
                      <SelectTrigger className="rounded-xl border-light-border">
                  ...
                  <Textarea
                    placeholder="Speaking prompt text or questions..."
                    value={promptText}
                  ...
                  <Textarea
                    placeholder="Sample answer or response notes (optional)"
                    value={sampleAnswer}
