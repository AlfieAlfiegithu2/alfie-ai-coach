...
import { 
  Phone, Calendar, Menu, X, Hand, Compass, Baby, GraduationCap, 
  Shield, HeartHandshake, Rainbow, Puzzle, Rocket, Check, MessageCircle, 
  DoorOpen, MapPin, Send, Mail, Instagram, Facebook, Twitter, Map,
  ArrowRight, BookOpen, Headphones, PenTool, Mic, Volume2, FileText,
  Target, Star, Brain, Trophy, Languages, TrendingUp
} from "lucide-react";
...
const HeroIndex = () => {
  ...
  return (
    <div className="min-h-screen antialiased text-black bg-neutral-50" style={{fontFamily: "'Inter', ui-sans-serif, system-ui, -apple-system, 'Segoe UI', Roboto, Helvetica, Arial, 'Apple Color Emoji', 'Segoe UI Emoji'"}}>
      {/* Header */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 mt-8">
            {testTypes.map((test, index) => (
              <article key={index} className="group overflow-hidden bg-black/5 border-black/10 border rounded-2xl shadow-xl backdrop-blur-none">
        ...
        <div className="sm:px-6 lg:px-8 max-w-7xl mr-auto ml-auto pr-4 pl-4">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {testimonials.map((testimonial, index) => (
              <article key={index} className="rounded-2xl border p-6 border-black/10 bg-black/5">
            ...
            <div className="grid md:grid-cols-2">
              <div className="sm:p-10 pt-8 pr-8 pb-8 pl-8">
                ...
                <div className="mt-6 space-y-4">
                  <button 
                    onClick={handleAuthAction}
                    className="w-full inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-semibold transition bg-black text-neutral-100 hover:bg-black/90 font-nunito"
                  >
                  ...
                  <button 
                    onClick={() => navigate('/ielts-portal')}
                    className="w-full inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-3 text-sm font-medium transition border-black/10 bg-black/0 text-black/90 hover:bg-black/5 font-nunito"
                  >
