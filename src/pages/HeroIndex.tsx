  ...
  return <div className="min-h-screen relative overflow-hidden">
      ...
      <header className="relative z-10 px-8 py-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <a href="#features" className="text-black hover:text-gray-600 transition-colors font-medium">Features</a>
            <a href="#tests" className="text-black hover:text-gray-600 transition-colors font-medium">Tests</a>
            <a href="#community" className="text-black hover:text-gray-600 transition-colors font-medium">Community</a>
          </nav>

          {/* Button Group */}
          <div className="flex items-center gap-4">
            <Button onClick={() => navigate('/auth')} variant="ghost" className="text-black transition-all duration-200 bg-white/20 hover:bg-white/30">
              Log In
            </Button>
      ...
      <section className="relative z-10 px-8 py-20">
        <div className="max-w-4xl mx-auto text-center">
          {/* Empty space where hero used to be */}
        </div>
      </section>
      ...
      <section id="tests" className="relative z-10 px-8 py-20">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {testTypes.map((test, index) => <Card key={index} className="bg-white/10 border-white/20 backdrop-blur-xl hover:bg-white/15 hover:shadow-[0_0_30px_rgba(59,130,246,0.5)] transition-all cursor-pointer group shadow-lg hover:shadow-2xl hover:scale-105 duration-300" onClick={() => navigate(test.path)}>
                <CardHeader>
      ...
      <footer className="relative z-10 px-8 py-12 border-t border-white/20 bg-black/30 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            ...
            {/* Tests Column */}
            <div>
              <h4 className="text-black font-semibold mb-3">Tests</h4>
              <div className="space-y-2 text-sm text-gray-600">
                <button onClick={() => navigate('/ielts-portal')} className="block hover:text-black transition-colors">IELTS Academic</button>
                <button onClick={() => navigate('/pte-portal')} className="block hover:text-black transition-colors">PTE Academic</button>
                <button onClick={() => navigate('/toefl-portal')} className="block hover:text-black transition-colors">TOEFL iBT</button>
                <button onClick={() => navigate('/general-portal')} className="block hover:text-black transition-colors">General English</button>
              </div>
            </div>
            
            {/* Features Column */}
            <div>
              <h4 className="text-black font-semibold mb-3">Features</h4>
              <div className="space-y-2 text-sm text-gray-600">
                <button onClick={() => navigate('/dashboard')} className="block hover:text-black transition-colors">AI Feedback</button>
                <button onClick={() => navigate('/dashboard')} className="block hover:text-black transition-colors">Progress Tracking</button>
                <button onClick={() => navigate('/community')} className="block hover:text-black transition-colors">Community</button>
                <button onClick={() => navigate('/pricing')} className="block hover:text-black transition-colors">Pricing</button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <button onClick={() => navigate('/dashboard')} className="block hover:text-black transition-colors">Help Center</button>
                <button onClick={() => navigate('/community')} className="block hover:text-black transition-colors">Contact Us</button>
                <button onClick={() => navigate('/admin/login')} className="block hover:text-black transition-colors">Admin Panel</button>
              </div>
            </div>
          </div>
