...
serve(async (req) => {
    ...
    try {
        ...
        const questionsToInsert = questions.map((q: any, i: number) => {
            ...
            // Get structure data - include question's own structure_data if provided
            let structureDataForQuestion = null;
            
            // If question has its own structure_data (from sections), use it
            if (q.structure_data) {
                structureDataForQuestion = q.structure_data;
            } else if (isPassageStart && passageData) {
                // Otherwise, use passage-level data for first question
