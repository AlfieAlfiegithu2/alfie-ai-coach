...
serve(async (req) => {
  ...
  try {
    ...
    // Create job row (admin seeding supported via service role header)
    const targetUserId = body?.asPublic ? user.id : user.id; // keep for future extension
    const { data: job, error: jobErr } = await supabase
      .from('jobs_vocab_seed')
      .insert({ user_id: targetUserId, total, status: 'running' })
      .select('*')
      .single();
    if (jobErr) throw jobErr;

    // Helper: fetch next frequency lemmas by level bucket
    async function fetchBatch(level: number, limit: number) {
      // naive: pick by rank ranges per level
      const ranges: Record<number, [number, number]> = {
        1: [1, 1500],
        2: [1501, 4000],
        3: [4001, 8000],
        4: [8001, 15000],
        5: [1501, 12000], // academic but not rare
      };
    ...
    for (const [levelStr, count] of Object.entries(levels)) {
      ...
      if (target <= 0) break;
      const batch = await fetchBatch(level, Math.max(10, Math.min(50, target))); // small chunk to avoid timeouts
      // ensure deck creation per 20
      ...
      for (const { lemma, rank } of items) {
        ...
        try {
          ...
          await supabase.from('vocab_cards').insert({
            ...
            language,
            level,
            is_public: !!body?.asPublic
          });
          deckCount += 1;
          completed += 1;
        } catch (e) {
          await supabase.from('jobs_vocab_seed').update({ last_error: String(e?.message || e), last_term: lemma }).eq('id', job.id);
        }
